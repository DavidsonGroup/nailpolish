use crate::duplicates::RecordPosition;
use crate::io::InMemoryRecord;
use crate::r#mod::ReadLocation;
use anyhow::{Context, Result};
use needletail::parser::{FastqReader, LineEnding, SequenceRecord};
use needletail::{parse_fastx_reader, FastxReader};
use regex::Regex;
use std::io::{BufReader, Cursor, Read, Seek, SeekFrom};
use std::path::Path;
use thiserror::Error;

/// A struct representing an in-memory sequence record.
/// It wraps a `Vec<u8>` containing the raw sequence data.
pub struct InMemorySequenceRecord(Vec<u8>);

impl InMemorySequenceRecord {
    /// Creates a new `InMemorySequenceRecord` from a given buffer.
    pub fn new(buf: Vec<u8>) -> Result<Self> {
        Ok(Self(buf))
    }

    /// Parses the sequence data and returns a `SequenceRecord`. This is a costly operation and
    /// should only be called once, since each invocation will call the parser again.
    pub fn seq(&self) -> Result<SequenceRecord> {
        let mut reader = FastqReader::new(&self.0);
        let rec = reader.next().context("Unexpected EOF")??;
        Ok(rec)
    }
}

const BUF_CAPACITY: usize = 1024usize.pow(2);

#[derive(Error, Debug)]
enum FileReadError {
    #[error("No reads were provided")]
    EmptyReadsProvided,
}

pub trait GroupedReadsAccessor {
    fn new(file: &Path) -> Result<Self>;
    fn fetch_reads(&mut self, reads: &[ReadLocation]) -> Result<Vec<InMemorySequenceRecord>>;
}

pub struct UncompressedFileReader {
    pub seq: SequentialReader,
    pub rnd: RandomAccessReader,
}

impl GroupedReadsAccessor for UncompressedFileReader {
    pub fn new(file: &Path) -> Result<Self> {
        Ok(Self {
            seq: SequentialReader::new(file)?,
            rnd: RandomAccessReader::new(file)?,
        })
    }

    pub fn fetch_reads(&mut self, reads: &[ReadLocation]) -> Result<Vec<InMemorySequenceRecord>> {
        let len = reads.len();

        // the first read (`head`) should be read using a sequential reader, while the
        // tail is not sequential and should be read using a random reader
        let (head, tail) = reads.split_at(1);
        let head = head.first().ok_or(FileReadError::EmptyReadsProvided)?;

        let first_read = self.seq.fetch(head)?;

        // create the remaining reads
        let mut result = Vec::with_capacity(len);
        result.push(first_read);
        for rem in tail {
            let read = self.rnd.fetch(rem)?;
            result.push(read);
        }

        Ok(result)
    }
}

pub trait SingleReadAccessor {
    fn new(file: &Path) -> Result<Self>;
    fn _fetch(&mut self, pos: u64, num_bytes: u64) -> Result<InMemorySequenceRecord>;

    fn fetch(&mut self, read: &ReadLocation) -> Result<InMemorySequenceRecord> {
        self._fetch(read.pos as u64, read.byte_len as u64)
    }
}

pub struct RandomAccessReader {
    file: std::fs::File,
}

impl SingleReadAccessor for RandomAccessReader {
    pub fn new(file: &Path) -> Result<Self> {
        let file = std::fs::File::open(file)?;
        Ok(Self { file })
    }

    fn _fetch(&mut self, pos: u64, num_bytes: u64) -> Result<InMemorySequenceRecord> {
        self.file
            .seek(SeekFrom::Start(pos))
            .with_context(|| format!("Unable to seek file at position {}", pos))?;

        let mut buffer = vec![0; num_bytes as usize];
        self.file
            .read_exact(&mut buffer)
            .with_context(|| format!("Could not read {num_bytes} bytes at position {pos}"))?;

        InMemorySequenceRecord::new(buffer)
    }
}

pub struct SequentialReader {
    reader: BufReader<std::fs::File>,
    position: u64,
}

impl SingleReadAccessor for SequentialReader {
    pub fn new(file: &Path) -> Result<Self> {
        let file = std::fs::File::open(file)?;
        let reader = BufReader::with_capacity(BUF_CAPACITY, file);

        Ok(Self {
            reader,
            position: 0,
        })
    }

    fn _fetch(&mut self, pos: u64, num_bytes: u64) -> Result<InMemorySequenceRecord> {
        let offset = (pos as i64) - (self.position as i64);
        if offset != 0 {
            warn!(
                "Offset is NOT zero:\n  {} -> {pos}, {num_bytes} bytes to read",
                self.position
            )
        }
        self.reader.seek_relative(offset)?;

        self.position = pos + num_bytes;

        let mut buffer = vec![0; num_bytes as usize];
        self.reader
            .read_exact(&mut buffer)
            .with_context(|| format!("Could not read {num_bytes} bytes at position {pos}"))?;

        InMemorySequenceRecord::new(buffer)
    }
}
